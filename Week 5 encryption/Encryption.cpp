...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    { // TODO: student need to change the next line from output[i] = source[i]
      ...
      // XOR each character in the source string with the corresponding character in the key
        output[i] = source[i] ^ key[i % key_length]; // XOR with key modded to account for key length
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    if (file.is_open())
    {
        ...
        file << timestamp << std::endl;           // Line 2: timestamp (yyyy-mm-dd)
        file << key << std::endl;                 // Line 3: key used
        file << data << std::endl;                // Line 4+: encrypted/decrypted data
        file.close();
    }
    else
    {
